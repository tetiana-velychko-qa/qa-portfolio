# Test Cases

This folder contains detailed test cases derived from the test scenarios, organized by feature/module.

## Test Suites

| Suite | File | Test Cases | Passed | Failed |
|---|---|---|---|---|
| Home Page | [TS-01_Home_Page.md](./TS-01_Home_Page.md) | 3 | 3 | 0 |
| Navigation | [TS-02_Navigation.md](./TS-02_Navigation.md) | 3 | 3 | 0 |
| Registration | [TS-03_Registration.md](./TS-03_Registration.md) | 5 | 4 | 1 |
| Login | [TS-04_Login.md](./TS-04_Login.md) | 3 | 3 | 0 |
| Forgot Password | [TS-05_Forgot_Password.md](./TS-05_Forgot_Password.md) | 2 | 2 | 0 |
| User Profile | [TS-06_User_Profile.md](./TS-06_User_Profile.md) | 3 | 2 | 1 |
| Pet Management | [TS-07_Pet_Management.md](./TS-07_Pet_Management.md) | 4 | 4 | 0 |
| Clinics | [TS-08_Clinics.md](./TS-08_Clinics.md) | 2 | 2 | 0 |
| Map | [TS-09_Map.md](./TS-09_Map.md) | 1 | 1 | 0 |
| Search and Filters | [TS-10_Search_and_Filters.md](./TS-10_Search_and_Filters.md) | 1 | 1 | 0 |
| Basic Security | [TS-11_Basic_Security.md](./TS-11_Basic_Security.md) | 2 | 2 | 0 |
| Accessibility | [TS-12_Accessibility.md](./TS-12_Accessibility.md) | 1 | 1 | 0 |

**Total: 30 test cases | 28 Passed | 2 Failed**

## Legend
- ✅ Passed — actual result matches expected result
- ❌ Failed — bug found, see linked report in `06_Bug_Reports/`
